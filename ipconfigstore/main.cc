...
static void write_dns(FILE *fp) {
    ...
    auto ndns = GetIntProperty("ro.boot.reaos_net_ndns", 0);
    for (int i = 1; i <= ndns; ++i) {
        dnsList.insert(GetProperty("ro.boot.reaos_net_dns" + std::to_string(i), ""));
    }
...
static void write_proxy(FILE *fp) {
    ...
    std::string proxy_type = GetProperty("ro.boot.reaos_net_proxy_type", "");
    if ("static" == proxy_type) {
        writePackedString("proxySettings", fp);
        writePackedString("STATIC", fp);

        writePackedString("proxyHost", fp);
        writePackedString(GetProperty("ro.boot.reaos_net_proxy_host", "").c_str(), fp);

        writePackedString("proxyPort", fp);
        writePackedUInt32(GetIntProperty("ro.boot.reaos_net_proxy_port", 3128), fp);

        writePackedString("exclusionList", fp);
        writePackedString(GetProperty("ro.boot.reaos_net_proxy_exclude_list", "").c_str(), fp);
    } else if ("pac" == proxy_type) {
        writePackedString("proxySettings", fp);
        writePackedString("PAC", fp);

        writePackedString("proxyPac", fp);
        writePackedString(GetProperty("ro.boot.reaos_net_proxy_pac", "").c_str(), fp);
